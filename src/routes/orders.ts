...
ordersRouter.post("/orders/:id/book", requireAuth, async (req, res) => {
  ...
  res.redirect(`/orders?status=pickupsAndManifests&updatedOrder=${encodeURIComponent(order.orderNumber)}`);
});
