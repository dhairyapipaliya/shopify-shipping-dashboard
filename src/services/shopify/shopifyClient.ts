...
export const fetchShopifyOrders = async (): Promise<ShopifyOrderPayload[]> => {
  if (env.SHOPIFY_USE_MOCK_DATA) return mockOrders();
  ...
  return withRetry(async () => {
    throw new Error("Shopify sandbox API integration pending for real orders + product dimensions/weight mapping.");
  }, { context: { provider: "shopify", operation: "orders_and_dimensions_sync" } });
};
